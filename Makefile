...
help:
	...
	@echo "  make docker-run            Web :4000 + MCP HTTP :9790 (ENABLE_MCP=1)"
...
docker-run-dev:
	@echo "Starting evaluator (foreground, compose --build). UI: http://localhost:4000 MCP: :9790"
...
docker-run-mcp-http:
	@echo "MCP HTTP is published on :9790 when ENABLE_MCP=1 (default with make docker-run)"
	@echo "Or: docker run --rm -p 9790:9790 $(HUB_IMAGE):dev mcp --http"
